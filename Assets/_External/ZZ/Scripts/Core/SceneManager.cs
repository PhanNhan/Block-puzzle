...
namespace ZZ
{
    ...
    public struct SceneData
    {
        ...
        public int IndexHierachy;
        public bool InStack;

        public SceneData(SceneType sceneType, Vector3 position, Data data, bool hasAnimation, int indexHierachy, bool inStack)
        {
            SceneType = sceneType;
            Position = position;
            Data = data;
            HasAnimation = hasAnimation;
            IndexHierachy = indexHierachy;
            InStack = inStack;
        }
    }
    ...
    public class SceneManager
    {
        ...
        public static void OnFadedOut(string sceneName, bool clearAll)
        {
            ...
            else
            {
                ...
                {
                    LoadUIPrefab(sceneName, m_Command.Count);
                }
            }
        }

        private static void LoadUIPrefab(string path, int indexHierachy)
        {
            GameObject obj = Resources.Load(m_pathParent + path) as GameObject;
            obj.transform.SetParent(m_UIParent);
            obj.transform.localScale = Vector3.one;
            obj.transform.localPosition = Vector3.zero;
            obj.transform.SetSiblingIndex(indexHierachy);
        }
        ...
        public static Controller Loading(bool active)
        {
            ActiveFixController(m_LoadingController, ref m_LoadingActive, active);
            return m_LoadingController;
        }
        ...
        public static void OnLoaded(Controller controller)
        {
            ...
            ZPool.Assets.ClearAllCacheAssets();
            System.GC.Collect();
        ...
        static void SetFixController(Controller controller, string sceneName, SceneType sceneType, Vector3 position, int minDepth)
        {
            if (controller == null)
            {
                ...
                LoadUIPrefab(sceneName, minDepth);
            }
        }
        ...
        {
            Debug.Log("Add scene name : " + sceneName);
            foreach (var item in m_Command)
            {
                Debug.LogError("Scene name: " + item.Key);
            }
            if (m_Command.ContainsKey(sceneName))
            {
                Debug.LogError("Same key! That same key value is : " + sceneName);
            }
        ...
        static void ExcuteController(Controller controller, SceneData sceneData)
        {
            ...
            controller.Supporter.ResortDepth(sceneData.IndexHierachy);
